...
function batchAnalyze(in, out){
	...
	for(i=0; i<files.length; i++){
		...
		open(in+files[i]);
		locateStructures(medRad, gaussRad, subBkgd, tolerance, quantRad);
		quantify(quantRad);
...
//----------------Locate structures----------------
function locateStructures(medRad, gaussRad, subBkgd, tolerance, enlarge){
...
function quantify(quantRad){
	...
	Table.applyMacro("Loading=Channel_1_Time_2-Channel_1_Time_1");
	Table.applyMacro("Release=Channel_1_Time_3-Channel_1_Time_4");

	Table.applyMacro("Bkgd_Corrected_Loading=Bkgd_Corrected_Channel_1_Time_2-Bkgd_Corrected_Channel_1_Time_1");

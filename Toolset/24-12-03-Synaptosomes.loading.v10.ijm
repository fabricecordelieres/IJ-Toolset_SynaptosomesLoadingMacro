...
var detectionArray=newArray("Channel1", "Channel2", "Channel3", "Channels2&3", "All");
var detectionDupArg=newArray(" channels=1", " channels=2", " channels=3", " channels=2-3", "");
var detectChNb=2;
...
macro "Register HyperStack Manually Action Tool - "{
	in=getDirectory("Where is the compiled data ?");
...
function applyRegistration(x, y, img){
	...
	for(i=1; i<=channels; i++){
		for(j=1; j<=frames; j++){
			...
			deltaX[j-1]=abs(x[0]-x[j-1]);
			deltaY[j-1]=abs(y[0]-y[j-1]);
		}
		resetMinAndMax;
	}
	Array.getStatistics(deltaX, xmin, xmax, xmean, xstdDev);
	Array.getStatistics(deltaY, ymin, ymax, ymean, ystdDev);
	makeRectangle(xmax, ymax, getWidth-2*xmax, getHeight-2*ymax);
...
function GUIAnalysis(){
	...
	Dialog.addChoice("Perform detection on", detectionArray, detectionArray[detectChNb]);
	...
	Dialog.addNumber("Donut thickness for local bkgd. quantif.", bkgdDonut);
	Dialog.show();

	detectCh=Dialog.getChoice();
	medRad=Dialog.getNumber();
	gaussRad=Dialog.getNumber();
	subBkgd=Dialog.getNumber();
	tolerance=Dialog.getNumber();
	quantRad=Dialog.getNumber();
	bkgdDonut=Dialog.getNumber();
	
	//Computes the position of the selected option within the array
	detectChNb=-1;
	
	for(i=0; i<detectionArray.length; i++){
		if(detectionArray[i]==detectCh){
			detectChNb=i;
			break;
		}
	}
...
function batchAnalyze(in, out){
	files=getSpecificFilesList(in, "_registered.tif");

	for(i=0; i<files.length; i++){
		roiManager("Reset");

		open(in+files[i]);
		locateStructures(detectChNb, medRad, gaussRad, subBkgd, tolerance, quantRad);
		quantify(quantRad);
...
//----------------Locate structures----------------
function locateStructures(detectOnCh, medRad, gaussRad, subBkgd, tolerance, enlarge){
	run("Duplicate...", "title=tmp duplicate"+detectionDupArg[detectOnCh]);
	...
	//Removes points at the border of the image to avoid error message when generating the band
	exclusionBand=2*(quantRad+bkgdDonut)+1;
	deletePointsOutsideRoi(xpoints, ypoints, 0, 0, 0, getWidth(), exclusionBand);
	getSelectionCoordinates(xpoints, ypoints);
	deletePointsOutsideRoi(xpoints, ypoints, 0, 0, getHeight()-1-exclusionBand, getWidth(), exclusionBand);
	getSelectionCoordinates(xpoints, ypoints);
	deletePointsOutsideRoi(xpoints, ypoints, 0, 0, 0, exclusionBand, getHeight());
	getSelectionCoordinates(xpoints, ypoints);
	deletePointsOutsideRoi(xpoints, ypoints, 0, getWidth()-1-exclusionBand, 0, exclusionBand, getHeight());
	getSelectionCoordinates(xpoints, ypoints);
	
	rejectStructures(enlarge);
}


//----------------Locate structures----------------
function rejectStructures(enlarge){
	getSelectionCoordinates(xpoints, ypoints);
	...
	run("Select None");
	setTool("rectangle");
	waitForUser("1-Click on Ok,\n2-Draw rectangles over the structures to suppress\n3-End by pressing the space bar");

	while(!isKeyDown("Space")){
		showStatus("Press space to stop removing detections");
		getCursorLoc(x, y, z, modifiers);
		if(selectionType()!=-1 && modifiers&16==0){
			getBoundingRect(x, y, width, height);
			deletePointsOutsideRoi(xpoints, ypoints, enlarge, x, y, width, height);
			getSelectionCoordinates(xpoints, ypoints);
			run("Enlarge...", "enlarge="+enlarge);
			addToOverlay();
			run("Select None");
...
//--Adds the current selection to the overlay on all timepoints--
...
//----------------Delete structures outside a ROI----------------
function deletePointsOutsideRoi(xpoints, ypoints, enlarge, x, y, width, height){
	xInside=newArray();
	yInside=newArray();
	...
	for(i=0; i<xpoints.length; i++){
		...
		if(!Roi.contains(xpoints[i], ypoints[i])){
			xInside=Array.concat(xInside, xpoints[i]);
			yInside=Array.concat(yInside, ypoints[i]);
		}
	}

	run("Select None");
	run("Remove Overlay");
	makeSelection("point", xInside, yInside);
}
...
function quantify(quantRad){
	...
	Stack.getDimensions(width, height, channels, slices, frames);
	run("Properties...", "unit=unit pixel_width=1.0000 pixel_height=1.0000 voxel_depth=1.0000");
	...
	for(r=0; r<xpoints.length; r++){
		...
		line=nResults;
		setResult("Label", line, "Detection_"+(r+1));
		...
		for(c=1; c<=channels; c++){
			for(t=1; t<=frames; t++){
				...
				getStatistics(area, mean, min, max, std, histogram);
				setResult("Channel_"+c+"-Time_"+t, line, area*mean);

				run("Make Band...", "band="+bkgdDonut);
				getStatistics(areaBkgd, meanBkgd, minBkgd, maxBkgd, stdBkgd, histogramBkgd);
				List.setMeasurements
				median=List.getValue("Median");
				setResult("Bkgd_Channel_"+c+"-Time_"+t, line, area*median);

				setResult("Bkgd_Corrected_Channel_"+c+"-Time_"+t, line, area*(mean-median));
			}
		}
	...
	Table.applyMacro("Loading=Channel_1_Time_3-Channel_1_Time_1");
	Table.applyMacro("Release=Channel_1_Time_3-Channel_1_Time_4");

	Table.applyMacro("Bkgd_Corrected_Loading=Bkgd_Corrected_Channel_1_Time_3-Bkgd_Corrected_Channel_1_Time_1");
	Table.applyMacro("Bkgd_Corrected_Release=Bkgd_Corrected_Channel_1_Time_3-Bkgd_Corrected_Channel_1_Time_4");
...
//----------------Add background quantification ROIs to ROI Manager----------------
function addBkgdROI(){
